...
@Component({
  selector: 'app-auth-entreprise',
  templateUrl: './auth-entreprise.component.html',
  styleUrls: ['./auth-entreprise.component.css']
})
export class AuthEntrepriseComponent {
    ...
    if (this.email == '') {
      ...
      this.authService.login(this.email, this.mot_de_passe).subscribe(
        (response) => {
          ...
          localStorage.setItem('access_token', JSON.stringify(response.token).replace(/['"]+/g, ''));
        ...
        (error) => {
          ...
          if (error.status === 401) {
            this.alertMessage('error', 'Erreur', 'Adresse e-mail ou mot de passe incorrect');
          } else {
            this.alertMessage('error', 'Erreur', 'Erreur d\'authentification. Veuillez réessayer.');
          }
        }
      );
    }
  ...
  logout() {
    ...
    this.router.navigate(['/authEntreprise']);
  }
  ...
  inscriptionUser() {
    if (this.nom_complet == '') {
      ...
      this.alertMessage(
        'error',
        'Attention',
        'le mot de ppasse doit etre supérieur ou égal à 10!'
      );
    ...
    } else if (this.numero_identification_naitonal == '') {
      this.alertMessage(
        'error',
        'Attention',
        'Merci de renseigner votre numéro d\'identification!'
      );
    } else if (this.numero_identification_naitonal.length < 10) {
      this.alertMessage(
        'error',
        'Attention',
        'Le numéro d\'identification doit être supérieur à 10!'
      );
    } else {
      let newUser: User = {
      ...
      this.authService.inscriptionEntreprise(newUser).subscribe(
        (addedUser) => {
          this.alertMessage(
            'success',
            'Super',
