...
export class AuthComponent implements OnInit {
    ...
    if (this.email == '') {
      ...
      this.authService.login(this.email, this.mot_de_passe).subscribe(
        ...
        (error) => {
          ...
          if (error.status === 401) {
            // Si le statut HTTP est 401 (Unauthorized), affichez un message d'erreur approprié
            this.alertMessage(
              'error',
              'Erreur',
  ...
  inscriptionUser() {
    ...
    if(this.exactNom && this.exactDescription && this.exactEmail && this.exactPassword && this.exactDescription) {
      let newUser: User = {
